...
public class S2Ghost: MonoBehaviour {
	...
	void Start () {
		...
		if(!cpy){
			PlayingManager.bossCo = 4; //編號4幽靈
			//對話
			mesTimer = 5f;
			if(!GameManager.s2BSee [0]){
				mesText = "....................";
				GameManager.s2BSee [0] = true;
	...
	void FixedUpdate () {
		if (worked) {
			...
			*/
				if(this.transform.GetChild(0).GetChild(1).renderer.material.color != iniColor)
					this.transform.GetChild(0).GetChild(1).renderer.material.color = Color.Lerp(this.transform.GetChild(0).GetChild(1).renderer.material.color,iniColor,0.1f);
				else
					blinkRed=false;
			}
			if(hitCD > 0f)
			...
			if(!phase2 && PlayingManager.phase > 2 && !cpy){
				iniColor = new Color(1f,1f,1f,0.784f); //原來77 52 79 140
				this.transform.GetChild(0).GetChild(1).renderer.material.color = Color.red;
				blinkRed=true;
				this.transform.GetChild(1).GetComponent<ParticleEmitter>().minEmission = 100;
				this.transform.GetChild(1).GetComponent<ParticleEmitter>().maxEmission = 100;
				phase2=true;
				AudioSource.PlayClipAtPoint(p2SE,Vector3.one);
			}
			//計算拍數
	...
	void OnGUI (){
		...
		if(showMes){
			...
			GUI.Window(4,mesRect,mesWindow,GameManager.BossName(4));
		}
	}
