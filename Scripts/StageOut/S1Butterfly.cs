...
public class S1Butterfly : MonoBehaviour {
	bool blinkRed=false,worked=true;
	Transform player,butterfly;
	//拍數,不連發
	int pi=0,tmpPi=0;
	...
	void Start () {
		butterfly = this.transform.GetChild (1);
		player = UserScreen.player;
		FireBall.bossFireBall=false;
		if(!cpy){
			PlayingManager.bossCo = 2; //編號2蝴蝶
			//對話
			mesTimer = 5f;
			if(!GameManager.s1BSee [1]){
				mesText = "這個星球的生物嗎?";
				GameManager.s1BSee [1] = true;
			}else{
	...
	void FixedUpdate () {
		if(worked){
			...
			//登場
			//1~3拍 移至位置看人
			//this.animation.CrossFade("fly",0.1f);
			if(stageStep==0){
				//位移
				if(pi==1)
					transform.position = Vector3.Lerp(transform.position,new Vector3(-1f,1f,0.0f),0.1f);
				else
			...
			//5~9拍 原地發單
				...
				if(tmpPi!=pi){
					...
					if(pi%2==0){
						//向玩家發射
						if(pi<24){
							Instantiate(fireBall[Random.Range(0,fireBall.Length-1)],this.transform.position,Quaternion.identity);
							PiSE();
							PlayingManager.outCount++;
						}else{
							//兩發
							Instantiate(fireBall[Random.Range(0,fireBall.Length-1)],new Vector3(this.transform.position.x+0.7f,this.transform.position.y,moveZ),Quaternion.identity);
							Instantiate(fireBall[Random.Range(0,fireBall.Length-1)],new Vector3(this.transform.position.x-0.7f,this.transform.position.y,moveZ),Quaternion.identity);
							PiSE();
							PlayingManager.outCount+=2;
						}
						moveX = Random.Range(-6.5f,6.5f);
						moveY = Random.Range(2.5f,6f);
					}
				}
				//看人
				transform.rotation = Quaternion.Lerp(transform.rotation,Quaternion.LookRotation(relativePos),0.2f);
			...
			//21~25拍 三連
				...
				if(tmpPi!=pi){
					...
					if(pi%2==0){
						...
						//向玩家發射
						//三發
						Instantiate(fireBall[fireBall.Length-1],new Vector3(this.transform.position.x,this.transform.position.y,moveZ),Quaternion.LookRotation(relativePos+Vector3.right));
						Instantiate(fireBall[fireBall.Length-1],new Vector3(this.transform.position.x,this.transform.position.y,moveZ),Quaternion.LookRotation(relativePos));
						Instantiate(fireBall[fireBall.Length-1],new Vector3(this.transform.position.x,this.transform.position.y,moveZ),Quaternion.LookRotation(relativePos-Vector3.right));
						PiSE();
				...
				if(pi%2==0){
					...
					if(changeFlag2>0)
						...
						if(changeFlag3<3){ //三發,每發0.142秒
							Instantiate(fireBall[fireBall.Length-1],new Vector3(this.transform.position.x,this.transform.position.y,moveZ),Quaternion.LookRotation(relativePos+new Vector3(0.7f,0f,0f)));
							Instantiate(fireBall[fireBall.Length-1],new Vector3(this.transform.position.x,this.transform.position.y,moveZ),Quaternion.LookRotation(relativePos));
							Instantiate(fireBall[fireBall.Length-1],new Vector3(this.transform.position.x,this.transform.position.y,moveZ),Quaternion.LookRotation(relativePos+new Vector3(-0.7f,0f,0f)));
							PiSE();
							PlayingManager.outCount+=3;
			...
			//45~49拍 隨機2
				...
				Vector3 relativePos = player.position - this.transform.position;
				//移動
				transform.position = Vector3.Lerp(transform.position,new Vector3(moveX,moveY,0.0f),0.1f);
				if(tmpPi!=pi){
					tmpPi=pi;
					if(pi%2==0){
						//發射
						moveY = Random.Range(2.5f,4.0f);
						//向玩家發射
						Instantiate(fireBall[Random.Range(0,fireBall.Length-1)],new Vector3(this.transform.position.x+1,this.transform.position.y,moveZ),Quaternion.identity);
						Instantiate(fireBall[Random.Range(0,fireBall.Length-1)],new Vector3(this.transform.position.x-1,this.transform.position.y,moveZ),Quaternion.identity);
						Instantiate(fireBall[Random.Range(0,fireBall.Length-1)],new Vector3(this.transform.position.x,this.transform.position.y,moveZ),Quaternion.identity);
						PiSE();
						PlayingManager.outCount+=3;
						moveX=Random.Range(-6.5f,6.5f);
					}
...
`					*/
					...
					if(pi>241){ //第一拍+最後不射
						if(pi<255){
							...
							moveY=4.0f;
							Instantiate(fireBall[fireBall.Length-1],new Vector3(this.transform.position.x,this.transform.position.y,moveZ),Quaternion.LookRotation(relativePos+Vector3.right));
							Instantiate(fireBall[fireBall.Length-1],new Vector3(this.transform.position.x,this.transform.position.y,moveZ),Quaternion.LookRotation(relativePos+Vector3.left));
							PiSE();
							PlayingManager.outCount+=2;
						}else{
							moveY=3.0f;
			...
			*/
				if(butterfly.renderer.material.color != Color.white)
					butterfly.renderer.material.color = Color.Lerp(butterfly.renderer.material.color,Color.white,0.1f);
				else
					blinkRed=false;
			}
			if(hitCD>0f)
			...
			if (PlayingManager.endIn) {
				...
				butterfly.renderer.material.color = Color.gray;
				GameObject testV;
	...
	void OnGUI (){
		...
		if(showMes){
			...
			GUI.Window(4,mesRect,mesWindow,GameManager.BossName(2));
		}
	}
	...
	void OnTriggerEnter(Collider other){
		if (Time.timeScale != 0) {
			...
			if(other.tag=="sticks" && hitCD<=0){
				...
				if(!cpy){
					butterfly.renderer.material.color = Color.red;
					blinkRed=true;
				}
