...
public class S3Pegasus : MonoBehaviour {
	bool blinkRed=false,phase2=false,worked=true;
	Transform player,pegasus;
	//拍數,不連發
	int pi,tmpPi=-1;
	//發射間隔
	...
	public GameObject boom,VBall,XBall,fourFireBall,sobi;
	//stage
	int stageStep,changeFlag;
	float moveX,moveZ=0f,moveY=1.0f;
	//模型
	Color iniColor = Color.white;
	...
	void Start () {
		pegasus = this.transform.GetChild(0);
		player = UserScreen.player;
		FireBall.bossFireBall=false;
		if(!cpy){
			PlayingManager.bossCo = 7; //編號7飛馬
			//對話
			mesTimer = 7f;
			if(!GameManager.s3BSee [0]){
				mesText = "聽說你是個很厲害的角色呢！";
				GameManager.s3BSee [0] = true;
	...
	void FixedUpdate () {
		if (worked) {
			...
			//看人
			Vector3 relativePos = new Vector3(player.position.x,player.position.y,-8f)-this.transform.position;
			this.transform.rotation = Quaternion.Lerp(this.transform.rotation,Quaternion.LookRotation(relativePos),0.1f);
			//**********行為*******************
			//1~5拍 空等
			if(stageStep==0){
				if(pi==9){
			...
			//9~13拍 左右轉四方
				...
				//移動
				if(changeFlag==0){
					this.transform.position = Vector3.Lerp(this.transform.position,new Vector3(8.0f,1f,moveZ),0.1f);
				}else{
					this.transform.position = Vector3.Lerp(this.transform.position,new Vector3(-8.0f,1f,moveZ),0.1f);
				}
					...
					if(pi!=110){
						if(pi%2==0){
							...
							if(pi>102){
								...
								Instantiate(fireBall[0],this.transform.position,new Quaternion(0,0,(Random.Range(-1.0f,-7.8f)),(Random.Range(-1.0f,1.0f))));
								PiSE();
								PlayingManager.outCount++;
							}
						}else{
			...
			//閃紅恢復
			if (blinkRed) {
				if(pegasus.renderer.material.color == iniColor){
					blinkRed=false;
				}else{
					pegasus.renderer.material.color = Color.Lerp(pegasus.renderer.material.color,iniColor,0.1f);
				}
			}
			if(hitCD > 0f)
				hitCD -= 0.02f;
			...
			if (PlayingManager.endIn) {
				...
				pegasus.renderer.material.color = Color.gray;
				GameObject testV;
			...
			if(!phase2 && PlayingManager.phase > 2 && !cpy){
				...
				if(!cpy){
					iniColor = new Color(0.207f,0.572f,1f,1f); //53 146 255 255
					pegasus.renderer.material.color = Color.red;
					blinkRed=true;
				}
				GameObject testV;
	...
	void OnGUI (){
		...
		if(showMes){
			...
			GUI.Window(4,mesRect,mesWindow,GameManager.BossName(7));
		}
	}
