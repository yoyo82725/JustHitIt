...
public class S2Candle : MonoBehaviour {
	...
	public GameObject boom,cube,fourBall,vBall,roundBall;
	Transform player;
	//stage
	...
	void Start () {
		...
		if(!cpy){
			PlayingManager.bossCo = 5; //編號5燭台
			//對話
			mesTimer = 5f;
			if(!GameManager.s2BSee [1]){
				mesText = "發現有趣的東西了！！";
				GameManager.s2BSee [1] = true;
			}else{
		...
		if (worked) {
			...
			//9~26拍 POSE
				...
				//旋轉
					...
					transform.Rotate(1f,((changeFlag<50)?changeFlag++:changeFlag),1f);
				}
				//看人
				else if(pi<72){
					transform.rotation = Quaternion.Lerp(transform.rotation,Quaternion.LookRotation(new Vector3(player.position.x,player.position.y,-8f)),0.2f);
				}
				//隱形
				else if(pi<80){
					transform.position = Vector3.Lerp(transform.position,new Vector3(moveX,3.5f,-12.0f),0.1f);
			...
			//26~34拍 風+隱出掃射
				...
				if(tmpPi!=pi){ //每一拍
					...
					if(pi<103){
						if(pi%2==0){
							if(phase2){
								Instantiate(cube,new Vector3(transform.position.x+1,transform.position.y,fireZ),Quaternion.identity);
								Instantiate(cube,new Vector3(transform.position.x-1,transform.position.y,fireZ),Quaternion.identity);
								PiSE();
								PlayingManager.outCount+=2;
							}else{
								Instantiate(cube,new Vector3(transform.position.x,transform.position.y,fireZ),Quaternion.identity);
								PiSE();
					...
					if(pi<170){
						...
						if(pi!=184){
							if(pi%2==0){
								if(phase2){
									Instantiate(fireBall[Random.Range(0,fireBall.Length-1)],new Vector3(transform.position.x+1,transform.position.y,fireZ),Quaternion.identity);
									Instantiate(fireBall[Random.Range(0,fireBall.Length-1)],new Vector3(transform.position.x,transform.position.y+1,fireZ),Quaternion.identity);
									Instantiate(fireBall[Random.Range(0,fireBall.Length-1)],new Vector3(transform.position.x,transform.position.y-1,fireZ),Quaternion.identity);
									Instantiate(fireBall[Random.Range(0,fireBall.Length-1)],new Vector3(transform.position.x-1,transform.position.y,fireZ),Quaternion.identity);
									PiSE();
									PiSE();
			...
			//74~82拍 移過玩家上方 275
				...
				//移至左下
					...
					this.transform.position = new Vector3(-15.0f,-1f,0f);
				}
				//左下至右下移動
				else if(pi<285){
					this.transform.position = Vector3.Lerp(transform.position,new Vector3(15.0f,-1f,0f),0.04f);
				}
				//移至左上
				else if(pi<286){
					this.transform.position = new Vector3(-15.0f,5.2f,0f);
			...
			if (blinkRed) {
				/* 原始
			if(this.transform.GetChild(0).renderer.material.color.r > 254){
				for(int i=0;i<this.transform.childCount;i++){
					this.transform.GetChild(i).renderer.material.color = Color.white;
				}
				blinkRed=false;
			}else{
				for(int i=0;i<this.transform.childCount;i++){
					this.transform.GetChild(i).renderer.material.color = Color.Lerp(this.transform.GetChild(i).renderer.material.color,Color.white,0.1f);
				}
			}
			*/
				if(this.transform.GetChild(0).renderer.material.color != iniColor)
					this.transform.GetChild(0).renderer.material.color = Color.Lerp(this.transform.GetChild(0).renderer.material.color,iniColor,0.1f);
				else
					blinkRed=false;
			}
			if(hitCD > 0f)
			...
			if(!phase2 && PlayingManager.phase > 2 && !cpy){
				...
				this.transform.GetChild(0).renderer.material.color = Color.red;
				blinkRed=true;
				this.transform.GetChild(1).GetComponent<ParticleEmitter>().minEmission = 20;
				this.transform.GetChild(1).GetComponent<ParticleEmitter>().maxEmission = 20;
				phase2=true;
				AudioSource.PlayClipAtPoint(p2SE,Vector3.one);
			}
			//計算拍數
	...
	void OnGUI (){
		...
		if(showMes){
			...
			GUI.Window(4,mesRect,mesWindow,GameManager.BossName(5));
		}
	}
