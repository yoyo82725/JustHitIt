...
public class S1Eagle : MonoBehaviour {
	bool blinkRed=false,worked=true;
	Transform player,eagle;
	//拍數,不連發
	int pi=0,tmpPi=0;
	...
	void Start () {
		eagle = this.transform.GetChild (0);
		player = UserScreen.player;
		FireBall.bossFireBall=false;
		if(!cpy){
			PlayingManager.bossCo = 1; //編號1老鷹
			//對話
			mesTimer = 5f;
			if(!GameManager.s1BSee [0]){
				mesText = "人類！別再反抗了，乖乖受死吧！";
				GameManager.s1BSee [0] = true;
			}else if(GameManager.openLevel < 2){
				mesText = "又是你這傢伙！再次受死吧！";
			}else{
				int ran = Random.Range(1,4);
				if(ran == 1)
					mesText = "又是你這傢伙！這次會收拾你！";
				else if(ran == 2)
					mesText = "無謂的掙扎！受死吧！";
				else
					mesText = "這次可沒這麼容易了！";
	...
	void FixedUpdate () {
		if(worked){
			...
			//3~5拍 轉圈看人
				...
				//轉圈
				if(pi<10){
					eagle.animation.CrossFade("bow",0.5f);
					transform.Rotate(0,(changeFlag>50)?changeFlag:changeFlag++,0);
				}else{
					if (eagle.animation.IsPlaying("bow"))
						eagle.animation.CrossFade("open",0.1f);
					if (eagle.animation["open"].normalizedTime>0.95)
						eagle.animation.CrossFade("openFly",0.1f);
					//位移
					transform.position = Vector3.Lerp(transform.position,new Vector3(4.5f,4.5f,0.0f),0.1f);
					//看人
					transform.rotation = Quaternion.Lerp(transform.rotation,Quaternion.LookRotation(relativePos),0.2f);
			...
			//5~9拍 原地發單
				...
				eagle.animation.CrossFade("openFly",0.5f);
				//看人
				transform.rotation = Quaternion.Lerp(transform.rotation,Quaternion.LookRotation(relativePos),0.2f);
			...
			//29~33拍 POSE
				...
				//動畫
				if(pi==121){
					eagle.animation.CrossFade("bow",0.2f);
				}else{
					eagle.animation.CrossFade("openFly",0.1f);
				}
			...
			//41~45拍 滑行三連 150
				...
				eagle.animation.CrossFade("openFly",0.0f);
				//看人
				this.transform.rotation = Quaternion.Lerp(this.transform.rotation,Quaternion.LookRotation(relativePos),0.2f);
...
`					*/
			...
			*/
				if(this.transform.GetChild(0).GetChild(1).renderer.material.color != Color.white)
					this.transform.GetChild(0).GetChild(1).renderer.material.color = Color.Lerp(this.transform.GetChild(0).GetChild(1).renderer.material.color,Color.white,0.1f);
				else
					blinkRed=false;
			}
			if(hitCD > 0f)
			...
			if (PlayingManager.endIn) {
				...
				eagle.animation.Play("bow");
				this.transform.GetChild(0).GetChild(1).renderer.material.color = Color.gray;
				GameObject testV;
	...
	void OnGUI (){
		...
		if(showMes){
			...
			GUI.Window(4,mesRect,mesWindow,GameManager.BossName(1));
		}
	}
